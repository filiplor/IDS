...
------ PROCEDURY
-- procedura která vypíše odkud pochází zákazníci z èr 
CREATE OR REPLACE PROCEDURE info_uzivatel
AS
jmeno zakaznik.jmeno%type;
ulice zakaznik.ulice%type;
cislo zakaznik.cislo_popisne%type;
mesto zakaznik.mesto%type;
psc   zakaznik.psc%type;
zeme  zakaznik.zeme%type;
CURSOR our_cur is SELECT jmeno, ulice, cislo_popisne, mesto, psc,zeme FROM zakaznik;
BEGIN
    DBMS_OUTPUT.put_line('Informace odkud pochází zakazníci z Èr ');
    IF our_cur %ISOPEN THEN
        CLOSE our_cur ;
    END IF;
    OPEN our_cur;
    LOOP
        FETCH our_cur INTO jmeno, ulice, cislo, mesto, psc, zeme;
        EXIT WHEN our_cur%notfound;
        IF zeme = 'Èr' THEN
            DBMS_OUTPUT.put_line('Zákazník '|| jmeno ||' bydlí v ulici '|| ulice||' èíslo popisné '|| cislo|| ' ve mìstì '|| mesto|| ' s poštovním smìrovacím èíslem '|| psc);
        END IF;
    END LOOP;
    CLOSE our_cur;
...
END;
/
-- vyhledá v databázi  zamìstnance a vypíše jeho typ smlouvy a typ opravneni, pokud takovi zamìstanec u nas nepracuje obsahuje ošetøení vyjímky pro neexistující hledání
CREATE OR REPLACE PROCEDURE info_zamestnanec(zamestnanec_login zamestnanec.login%type) AS
jmeno zamestnanec.jmeno%type;
smlouva zamestnanec.typ_smlouvy%type;
opravneni zamestnanec.typ_opravneni%type;
login zamestnanec.login%type;
BEGIN
    SELECT Z.jmeno, Z.typ_smlouvy, Z.typ_opravneni, Z.login
    INTO jmeno, smlouva, opravneni, login
    FROM zamestnanec Z
    WHERE Z.login = zamestnanec_login;
        DBMS_OUTPUT.put_line('Zamìstanec '|| jmeno ||' s typem smlouvy '|| smlouva || ' má oprávnení '|| opravneni);
    EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.put_line('Zamìstanec s loginem '|| login ||  ' u nás již nepracuje');
END;
/    
...
END;
/
CALL info_zamestnanec('xpepa98');
CALL info_zamestnanec('xolaf98');
...
------ MATERIALIZED VIEW
--
DROP MATERIALIZED VIEW our_mv;
CREATE MATERIALIZED VIEW our_mv
NOLOGGING
CACHE
BUILD IMMEDIATE
REFRESH ON COMMIT
...
AS
    SELECT jmeno FROM zakaznik z JOIN pravnicka_osoba p ON z.id_zakaznika = p.id_zakaznika;
------ PRIVILEGES
...
GRANT ALL ON objednavka TO xloren18;
GRANT ALL ON objednavka TO xloren18;
GRANT ALL ON our_mv TO xloren18;
GRANT ALL ON info_zamestnanec TO xloren18;
GRANT ALL ON info_uzivatel TO xloren18;
